...
# Resets the Cloudflare demo's attack-fixture file to its original (vulnerable,
# pre-mitigation) state. Run this after a fix PR for the Worker rate-limit or
# app-side detector merges, so the demo can run again from a clean baseline.
#
# Unlike the Datadog/Sentry demos, the Cloudflare demo's "attack" is entirely
# scripted in a fixture file (no real failing code path the user has to break).
# This script restores the fixture's stage curves to the documented baseline so
# any local edits an engineer made while iterating on the demo are reverted.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
FIXTURE_FILE="$REPO_ROOT/src/lib/demo/cloudflare-attack-fixture.ts"
...
if git diff --cached --quiet -- "$FIXTURE_FILE"; then
  echo "Cloudflare demo fixture is already at baseline. Nothing to commit."
  exit 0
fi
git commit -m "chore: reset cloudflare demo fixture for next run"
echo ""
echo "Demo fixture reset. Push to main to redeploy:"
