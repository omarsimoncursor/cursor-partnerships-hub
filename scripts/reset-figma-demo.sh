...
# Resets the Figma demo's drifted product card to its (drifted) state so the
# demo is repeatable. Run this AFTER a fix PR (e.g. PR #163) merges, then
# push to redeploy.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
DRIFT_FILE="$REPO_ROOT/src/components/figma-demo/product-card-drifted.tsx"

cat > "$DRIFT_FILE" << 'DRIFTEOF'
'use client';
...
cd "$REPO_ROOT"
git add "$DRIFT_FILE"
git commit -m "chore: reset figma demo drift for next run"
echo ""
echo "Demo drift reset. Push to main to redeploy:"
