...
# Resets the PagerDuty demo seed back to "bad deploy = true" so the next run
# fires the simulated v1.43.0 incident again. Run this after merging a real
# fix PR that flipped BAD_DEPLOY to false.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
SEED_FILE="$REPO_ROOT/src/lib/demo/payments-deploy.ts"
...
cd "$REPO_ROOT"
git add "$SEED_FILE"
git commit -m "chore: reset pagerduty demo seed for next run"
echo ""
echo "Demo seed reset. Push to main to redeploy:"
