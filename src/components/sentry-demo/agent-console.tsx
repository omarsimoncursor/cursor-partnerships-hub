...
type Channel = 'sentry' | 'github' | 'jira' | 'shell' | 'opus' | 'composer' | 'codex' | 'analysis' | 'codegen' | 'done';
...
const CHANNEL_STYLES: Record<Channel, { label: string; color: string; bg: string; border: string }> = {
  sentry:   { label: 'sentry-mcp',    color: 'text-[#b8a6ff]',    bg: 'bg-[#362D59]/25',         border: 'border-[#362D59]/40' },
  ...
  analysis: { label: 'analysis',      color: 'text-accent-amber', bg: 'bg-accent-amber/10',      border: 'border-accent-amber/20' },
...
const SCRIPT: Step[] = [
  // Intake
  { channel: 'sentry',   delayMs: 400,  label: 'Fetching issue details',          detail: 'GET /api/0/issues/{id}/' },
  { channel: 'sentry',   delayMs: 800,  label: 'TypeError in formatPaymentReceipt', detail: "Cannot read properties of undefined (reading 'brand')" },
  { channel: 'sentry',   delayMs: 600,  label: 'Parsing stack + breadcrumbs',     detail: 'format-payment.ts:5 → order-processor.ts:22 → CheckoutCard · 312 users affected' },

  // Jira ticket created early — triage is tracked
  { channel: 'jira',     delayMs: 700,  label: 'Creating incident ticket',        detail: 'Project: CUR · Type: Bug · Priority: P1' },
  { channel: 'jira',     delayMs: 500,  label: 'Ticket CUR-4291 created',         detail: 'Linked to Sentry event a2f1…4c0e' },

  // Opus triages
  { channel: 'opus',     delayMs: 1000, label: 'Claude Opus: triaging',           detail: 'Model selected for long-context reasoning over stack + recent history' },
  { channel: 'github',   delayMs: 700,  label: 'Pulling commit history',          detail: 'git log --since=7.days -- src/lib/demo/' },
  { channel: 'github',   delayMs: 600,  label: 'Regression found: a4f2e1b',       detail: '"feat: add bank_transfer payment type" · 2 days ago' },
  { channel: 'opus',     delayMs: 1100, label: 'Root-cause hypothesis formed',    detail: 'bank_transfer lacks `details` · PaymentMethod type is non-optional · 2 files need changes' },
  { channel: 'codegen',  delayMs: 900,  label: 'Generating triage report',        detail: 'docs/triage/2026-04-16-typeerror-format-payment.md' },

  // Composer writes
  { channel: 'composer', delayMs: 1100, label: 'Composer: writing the patch',     detail: 'Model selected for speed on scoped edits' },
  { channel: 'shell',    delayMs: 500,  label: 'Reading format-payment.ts + order-processor.ts', detail: '2 files · 47 lines loaded' },
  { channel: 'composer', delayMs: 900,  label: 'Patched PaymentMethod type',      detail: 'details?: { last4: string; brand: string }' },
  { channel: 'composer', delayMs: 900,  label: 'Added null guard in formatPaymentReceipt', detail: 'return payment.details ? formatCard(…) : "Bank transfer (ACH)"' },

  // Codex reviews
  { channel: 'codex',    delayMs: 1100, label: 'Codex: reviewing patch before PR', detail: 'Model selected for code review depth' },
  { channel: 'codex',    delayMs: 800,  label: 'No regressions detected',         detail: 'Callers of processOrder() unaffected · return type preserved' },
  ...
  { channel: 'shell',    delayMs: 900,  label: 'Reproducing in headless browser', detail: 'POST /partnerships/sentry/demo · click #process-order' },
  { channel: 'shell',    delayMs: 700,  label: 'Order processed — no error thrown', detail: '✓ fix verified live' },

  // PR submission
  { channel: 'github',   delayMs: 600,  label: 'Creating branch fix/bank-transfer-null-ref', detail: 'base: main' },
  { channel: 'github',   delayMs: 600,  label: 'Committing & pushing',            detail: '2 files changed · +7 −3 · signed-off-by: cursor-agent' },
  { channel: 'github',   delayMs: 800,  label: 'Opening pull request #142',       detail: 'fix: handle undefined payment details in order processor' },
  { channel: 'jira',     delayMs: 500,  label: 'CUR-4291 → In Review',            detail: 'PR #142 linked to ticket' },
  { channel: 'done',     delayMs: 500,  label: 'Artifacts ready for review',      detail: 'Triage report · Jira ticket · Pull request' },
];
...
// The script plays back in ~19.4s of real time for demo pacing, but the
// displayed timestamps are scaled to reflect realistic agent work time
// (~2m 14s of MCP round-trips, model inference, and shell commands).
    ...
    <div className="w-full h-full rounded-xl border border-dark-border bg-dark-surface overflow-hidden flex flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 font-mono text-xs space-y-1.5 min-h-[320px]">
        {visibleSteps.length === 0 && (
          <p className="text-text-tertiary italic px-1 py-2">Waiting for Sentry webhook…</p>
        )}
