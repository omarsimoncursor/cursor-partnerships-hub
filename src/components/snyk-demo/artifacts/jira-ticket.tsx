...
const JIRA_COMMENTS = [
  {
    ...
    avatarInitial: 'C',
    body: 'Automatically opened from Snyk issue SNYK-JS-CUSTOMER-PROFILE-001. Triage report attached, PR #214 submitted — exploit replay 12 leaked → 0 leaked, snyk test clean at the medium threshold. Awaiting AppSec review.',
    pill: 'Automation',
  },
  {
    author: 'Snyk',
    time: 'a few seconds ago',
    avatarBg: 'bg-[#4C44CB]/25',
    avatarColor: 'text-[#9F98FF]',
    avatarInitial: 'S',
    body: 'Critical NoSQL injection (CWE-943, CVSS 9.8) in src/lib/demo/customer-profile.ts:22. Tainted source: request.query.username. Sink: CUSTOMERS.filter on line 24. Companion finding: SNYK-JS-MONGOOSE-2961688 (High, 7.5).',
    pill: 'Integration',
  },
  {
    author: 'AppSec channel',
    time: 'a few seconds ago',
    avatarBg: 'bg-[#06AC38]/15',
    avatarColor: 'text-[#57D990]',
    avatarInitial: 'A',
    body: '#appsec-incidents notified. Channel auto-paged when Snyk severity ≥ critical. No human escalation needed while Cursor is patching.',
    pill: 'Integration',
  },
];

const TIMELINE: Array<{ time: string; status: string; by: string; tone: 'neutral' | 'amber' | 'blue' | 'green' }> = [
  { time: '09:14:02', status: 'Backlog', by: 'Snyk webhook', tone: 'neutral' },
  { time: '09:14:05', status: 'In Progress', by: 'Cursor Agent', tone: 'amber' },
  { time: '09:16:08', status: 'In Review', by: 'PR #214 opened', tone: 'blue' },
];
      ...
      <div className="relative w-full max-w-5xl max-h-[92vh] rounded-lg border border-[#2C333A] bg-[#1D2125] overflow-hidden flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-4 py-2 border-b border-[#2C333A] bg-[#161A1D] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[#7C8A99] text-xs">/</span>
            <span className="text-[13px] text-[#9FADBC]">CUR-7841</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-8 pt-5 pb-2 shrink-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[12px] text-[#7C8A99] hover:text-[#9FADBC] cursor-pointer">CUR-7841</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#E11D48]/15 border border-[#E11D48]/30 text-[#FB7185] font-medium">
              Security · Critical
            </span>
          </div>
          <h1 className="text-[22px] font-semibold text-[#B6C2CF] leading-tight">
            NoSQL injection in customer profile lookup — tainted query.username flows into Mongo selector
          </h1>
        </div>
            ...
            <div className="min-w-0">
              <section className="mb-7">
                ...
                <div className="text-[13.5px] text-[#B6C2CF] leading-relaxed space-y-3">
                  <p>
                    Snyk Code surfaced a <strong className="font-semibold text-[#FB7185]">Critical (CVSS 9.8)</strong> NoSQL injection finding in{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#9F98FF] font-mono">src/lib/demo/customer-profile.ts:22</code>{' '}
                    at <strong className="font-semibold">09:14 PDT</strong>. The morning AppSec replay returned the entire customer table — including hashed credentials and admin role flags — for a single crafted request.
                  </p>
                  <p>
                    Root cause:{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#F87462] font-mono">lookupCustomerProfile</code>{' '}
                    builds the Mongo selector by string-interpolating{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#4C9AFF] font-mono">query.username</code>{' '}
                    directly into the predicate. The canonical payload{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#FB7185] font-mono">{`' || '1'=='1`}</code>{' '}
                    collapses the parser to <em>always-true</em>, returning every record.
                  </p>
                  <p>
                    Companion finding: Snyk Open Source flagged{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#4C9AFF] font-mono">mongoose@5.13.7</code>{' '}
                    as vulnerable to <em>SNYK-JS-MONGOOSE-2961688 (prototype pollution, CVSS 7.5)</em>. Fixed in 5.13.20.
                  </p>
                  <p>
                    Regression introduced in commit{' '}
                    <code className="px-1 py-0.5 rounded bg-[#161A1D] border border-[#2C333A] text-[12px] text-[#4C9AFF] font-mono">5e9d3c2</code>{' '}
                    ...
                    {TIMELINE.map((t, i) => {
                      ...
                      return (
                        <li key={i} className="ml-4 last:mb-0">
                          ...
                          />
                          <div className="flex items-center gap-3">
                            <span className={`px-2 py-0.5 rounded border text-[11px] font-medium ${toneBg}`}>
                              {t.status}
                            </span>
              ...
              <section className="mb-7">
                ...
                <div className="space-y-1.5">
                  <LinkRow
                    type="blocks"
                    refLabel="PR #214"
                    title="security: parameterize customer profile lookup + bump mongoose"
                    status="Open"
                  ...
                  <LinkRow
                    ...
                    title="NoSQL injection · Snyk Code · CVSS 9.8"
                    status="Critical"
                    statusColor="bg-[#C9372C]"
                  />
                  <LinkRow
                    type="relates to"
                    refLabel="SNYK-JS-MONGOOSE-2961688"
                    title="Prototype pollution · Snyk Open Source · CVSS 7.5"
                    status="High"
                    statusColor="bg-[#A54800]"
                  />
                  <LinkRow
                    type="caused by"
                    refLabel="Commit 5e9d3c2"
                    title="feat: add internal customer lookup"
                    status="Merged"
            ...
            <aside className="min-w-0">
              <div className="rounded-lg border border-[#2C333A] bg-[#161A1D] overflow-hidden">
                ...
                <dl className="p-4 space-y-3 text-[12.5px]">
                  ...
                  <DetailRow
                    label="Reporter"
                    value="Snyk Integration"
                    avatar="S"
                    avatarBg="bg-[#4C44CB]/30"
                    avatarColor="text-[#9F98FF]"
                  />
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="flex items-center gap-1.5 text-[#B6C2CF]">
                      <ShieldAlert className="w-3 h-3 text-[#FB7185]" />
                      Security · Vulnerability
                    </dd>
                  </div>
                  <div>
                    <dt className="text-[#7C8A99] mb-1">CVSS</dt>
                    <dd className="text-[#FB7185] font-mono">9.8 · Critical</dd>
                  </div>
                  <div>
                    <dt className="text-[#7C8A99] mb-1">CWE</dt>
                    <dd className="text-[#B6C2CF] font-mono">CWE-943</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="text-[#B6C2CF]">customer-profile · auth-platform</dd>
                  </div>
                  <div>
              ...
              <div className="mt-4 rounded-lg border border-[#2C333A] bg-[#161A1D] overflow-hidden">
                ...
                <div className="p-4 space-y-2 text-[12.5px] text-[#B6C2CF]">
                  ...
                  <p>
                    ...
                    <span className="font-mono text-[11.5px]">snyk-webhook / project-snapshot</span>
                  </p>
                  <p>
                    <span className="text-[#7C8A99]">Resolution time </span>
                    <span className="font-mono text-[#57D9A3]">2m 03s</span>
                  </p>
                  <p className="pt-2 border-t border-[#2C333A] mt-2">
                    ...
                    <span className="text-[#57D9A3]">12 leaked → 0 leaked</span>
                  </p>
                </div>
              </div>
            </aside>
